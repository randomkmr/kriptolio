...
const descriptionsEntity = require("../database/entities/descriptions");
...
exports.postPortfolio = async (req, res) => {
  try {
    ...
    const user_id = Object.values(req.user)[0];
    const data = await portfolioEntity.postAsset(
      user_id,
      asset_code,
      buying_price,
      amount
...
exports.getDescription = async (req, res) => {
  try {
    ...
    const data = await descriptionsEntity.getDesc(asset_id);
    res.status(201).json({ ...data });
  } catch (error) {
    res.status(500).json({ error });
  }
